...
Send me a note at jcponcemath@gmail.com
 */
...
function draw() {
    ...
    for (let i = 0; i <= 2 * PI; i += PI / 50) {
        let xc = cos(i);
        let yc = sin(i);
        vertex(100 * (xc * (1 - n) + JkTransX(xc, yc) * n), -100 * (yc * (1 - n) + JkTransY(xc, yc) * n));
    }
    endShape(CLOSE)
...
function easing(x){
    return 1 / 2 * sin(PI / 2 * cos((x + TWO_PI + 0.9) / 2)) + 1 / 2;
}
...
class Particle {
    ...
    display() {
        ...
        noStroke();
        let n = easing(transf);
        this.updatex = map(this.x * (1 - n) + JkTransX(this.x, this.y) * n, -8, 8, -width, width);
        this.updatey = map(-this.y * (1 - n) - JkTransY(this.x, this.y) * n, -4, 4, -height, height);
        //pg.translate(20, 20);
