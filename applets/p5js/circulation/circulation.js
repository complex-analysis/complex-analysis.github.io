/* Circulation simulation designed with p5.js (https://p5js.org/)
...
function draw() {
    ...
    text('G = '+nfc(sliderk.value(),1),-40, 200);//for slider k
...
let U = 1;
let Ci = 0.1;
let P = (t, x, y) => ( (2 * a*a * U * y*y)/((x*x+ y*y)*(x*x+ y*y)) + U*(1 - (a*a)/(x*x + y*y)) - (sliderk.value()*y)/(2*PI*(x*x + y*y))  );//Change this function
let Q = (t, x, y) =>  (  -(2*a*a * U * x * y)/((x*x+ y*y)*(x*x+ y*y)) + (sliderk.value() * x)/(2*PI*(x*x + y*y)));//Change this function
...
function controls() {
    ...
    sliderk = createSlider(-30, 30, 1, 0.1);
