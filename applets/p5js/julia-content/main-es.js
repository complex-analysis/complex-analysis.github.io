...
function draw() {
    ...
    if (starting == false) {
        ...
        textAlign(LEFT);
        textSize(32);
        text("Usa los controles de la izquierda,", width / 2 - 150, height / 2 - 120);
        text("o las teclas correspondientes.", width / 2 - 150, height / 2 - 70);
        textAlign(CENTER);
        text("Haz clic aquí para comenzar.", width / 2 + 50, height / 2);
    }

    if (starting == true) {
        textAlign(LEFT);
        if (mouseX >= ctlsBack && mouseX <= width && mouseY >= 0 && mouseY <= height) {
            cursor('none');
        } else cursor(HAND);
        julia.update();
        julia.draw();
...
function mouseWheel(e) {
    ...
    if (starting == true && (mouseX >= ctlsBack && mouseX <= width && mouseY >= 0 && mouseY <= height)) {
...
function Julia() {
    ...
    this.maxIter = 90;//slider.value();//150;
    this.origZoom = 1;
...
Julia.prototype.draw = function () {
    ...
    textSize(24);
    if (mouseX > ctlsBack) {
        text("c es (" + str(round(c.x * 1000) / 1000.0) + "," + str(round(c.y * 1000) / 1000.0) + ")", ctlsBack + 10, height - 15);
    }
...
function controlsUI() {
    ...
    buttonRESET.position(buttonUP.x - 14, buttonUP.y + 200);
    ...
    buttonINFO.position(buttonUP.x - 8, buttonUP.y + 250);
    buttonINFO.addClass('button');
    buttonINFO.mousePressed(userINFO);

    sliderIter = createSlider(0, 150, 90, 1);
