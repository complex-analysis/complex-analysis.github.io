...
let WIDTH = 490;
let HEIGHT = 490;
let ctlsBack = 0;//This is just need it when controls are inside canvas
    ...
    plot() {
        ...
        if (this.printDebug) {
            ...
            text("x: " + str(round(this.pos.x * 1000) / 1000)
                ...
                + "\niteraciones: " + str(round((this.maxIter) * 1000) / 1000)
                , 5, 19
            );
        ...
        if (0 < mouseX && mouseX < width && 0 < mouseY && mouseY < height || changeC === false) {
            ...
            text("c es (" + str(round(c.x * 1000) / 1000.0) + "," + str(round(c.y * 1000) / 1000.0) + ")", 5, height - 15);
        } else {
