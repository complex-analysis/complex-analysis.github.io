/* Uniform flow around circle radius a simulation designed with p5.js (https://p5js.org/)
...
function draw() {
    ...
    if(starting==true) {
        ...
        for (let i=particles.length-1; i>=0; i-=1) {
            ...
            p.update();
            p.display();
            if ( p.x > frameWidth ||  p.y > frameHeight || p.x < -frameWidth ||  p.y < -frameHeight || sqrt(pow(p.x , 2) + pow(p.y , 2)) < r) {
    ...
    text('Drag Q = '+nfc(slidera.value(),1),-60, 200);//for slider Q
...
let P = (t, x, y) =>  sliderU.value() * ( 1 - pow(r, 2) * (x*x - y*y)/pow(x*x+y*y,2) );//Change this function
let Q = (t, x, y) =>  -2* sliderU.value() * pow(r, 2) * x * y / pow(x*x+y*y,2);//Change this function
...
function controls() {
    ...
    //slidera = createSlider(0, 1, 0, 0.1);
    //slidera.position(250, 460);
    //slidera.style('width', '150px');

    sliderU = createSlider(0.5, 3, 1, 0.1);
    sliderU.position(20, 360);
